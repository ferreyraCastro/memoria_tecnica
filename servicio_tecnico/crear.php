...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errores) {
        $stmt = $db->prepare("INSERT INTO mantenimientos (equipo_id, fecha, tipo, problema_detectado, descripcion, componentes_reemplazados, repuestos_utilizados, tecnico, observaciones)
            VALUES (?,?,?,?,?,?,?,?,?)");
        $stmt->execute([
            $form['equipo_id'], $form['fecha'], $form['tipo'], $form['problema_detectado'] ?: null, $form['descripcion'],
            $form['componentes_reemplazados'] ?: null, $form['repuestos_utilizados'] ?: null, $form['tecnico'] ?: null, $form['observaciones'] ?: null,
        ]);
        $_SESSION['flash_success'] = 'Registro de servicio técnico guardado.';
        redirect('../equipos/ver.php?id=' . $form['equipo_id']);
    }
}
...
$pageTitle = 'Registrar servicio técnico';
require_once __DIR__ . '/../includes/header.php';
require_once __DIR__ . '/../includes/layout_start.php';
?>

<div class="page-header"><h1><i class="bi bi-plus-lg"></i> Registrar servicio técnico</h1></div>
    ...
    <form method="post" novalidate>
      <div class="row g-3">
        ...
        <div class="col-md-6">
          ...
          <input type="text" name="componentes_reemplazados" class="form-control" value="<?= h($form['componentes_reemplazados']) ?>" placeholder="Ej: fuente, memoria RAM">
        </div>
        <div class="col-md-6">
