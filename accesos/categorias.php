...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['nombre'])) {
        ...
        if ($nombre === '') {
            ...
            try {
                $db->prepare('INSERT INTO categorias_accesos (nombre) VALUES (?)')->execute([$nombre]);
...
if (isset($_GET['eliminar'])) {
    $db->prepare('DELETE FROM categorias_accesos WHERE id = ?')->execute([(int)$_GET['eliminar']]);
...
if (isset($_GET['editar']) && isset($_GET['nombre'])) {
    ...
    if ($nuevoNombre === '') {
        ...
        try {
            $db->prepare('UPDATE categorias_accesos SET nombre = ? WHERE id = ?')->execute([$nuevoNombre, (int)$_GET['editar']]);
...
$categorias = $db->query('SELECT c.*, (SELECT COUNT(*) FROM accesos a WHERE a.categoria_id = c.id) usados FROM categorias_accesos c ORDER BY nombre')->fetchAll();
$flashSuccess = $_SESSION['flash_success'] ?? null; unset($_SESSION['flash_success']);

$pageTitle = 'Categorías de accesos';
require_once __DIR__ . '/../includes/header.php';
require_once __DIR__ . '/../includes/layout_start.php';
?>

<div class="page-header">
  <div><h1><i class="bi bi-tags"></i> Categorías de accesos</h1></div>
    ...
    <div class="card-app">
      <div class="table-responsive">
        <table class="table table-app mb-0">
          <thead><tr><th>Nombre</th><th>Accesos</th><th class="text-end">Acciones</th></tr></thead>
          <tbody>
