...
$stmt = $db->prepare('SELECT * FROM accesos WHERE id = ?');
$stmt->execute([$id]);
$acceso = $stmt->fetch();
if (!$acceso) { $_SESSION['flash_error'] = 'Acceso no encontrado.'; redirect('index.php'); }

$errores = [];
$form = [
    'servicio' => $acceso['servicio'], 'usuario' => $acceso['usuario'],
    'password' => decryptString($acceso['password_cifrada'], $acceso['iv']),
    'url' => $acceso['url'], 'categoria_id' => $acceso['categoria_id'],
    'observaciones' => $acceso['observaciones'], 'fecha_actualizacion' => $acceso['fecha_actualizacion'],
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errores) {
        ...
        $stmt = $db->prepare("UPDATE accesos SET servicio=?, usuario=?, password_cifrada=?, iv=?, url=?, categoria_id=?, observaciones=?, fecha_actualizacion=? WHERE id=?");
        $stmt->execute([
            $form['servicio'], $form['usuario'], $enc['data'], $enc['iv'], $form['url'] ?: null,
            $form['categoria_id'], $form['observaciones'] ?: null, $form['fecha_actualizacion'], $id,
        ]);
        $_SESSION['flash_success'] = 'Acceso actualizado correctamente.';
        redirect('index.php');
    }
}

$categorias = $db->query('SELECT * FROM categorias_accesos ORDER BY nombre')->fetchAll();
$pageTitle = 'Editar acceso';
require_once __DIR__ . '/../includes/header.php';
require_once __DIR__ . '/../includes/layout_start.php';
?>

<div class="page-header">
  <h1><i class="bi bi-pencil"></i> Editar acceso</h1>
</div>
    ...
    <form method="post" novalidate>
      ...
      <div class="row g-3">
        ...
        <div class="col-md-8">
          ...
          <input type="text" name="url" class="form-control" value="<?= h($form['url']) ?>">
        </div>
        <div class="col-md-4">
